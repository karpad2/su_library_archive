<template>
	...
	<md-card>	
		...
		<div class="section">
			<newspapercard v-for="newspaper in searchednewspapers" :key="newspaper.id" :newspaper_id="newspaper.id"/>
		</div>
		</md-card-content>
	</md-card>
	</div>
...
// {{gt("genres")}}
	...
	export default {
		...
		async mounted() {
			...
			{
				let q=query(collection(firestore,"newspapers"),limit(10));
		...
		methods: {
			async searching()
			{
				...
				let q=query(collection(firestore,"newspapers"),where("keywords","array-contains",[this.seaching_text]),limit(10));
				let c=await getDocs(q);
				c.forEach(element => {
				this.check_element_exist({id:element.id});
				});
