...
import NPages from "@/components/parts/Newspaper_pages";
...
import flag from "@/components/parts/flag";
	...
	export default {
		...
		async mounted() {
			...
			try{
        newspaper_ref=await getDocFromCache(doc(firestore,"newspapers",this.newspaper_id));
        this.newspaper=newspaper_ref.data();
		
        }
        catch(e)
        {
           newspaper_ref=await getDoc(doc(firestore,"newspapers",this.newspaper_id));
           this.newspaper=newspaper_ref.data(); 
        }
			this.newspaper=newspaper_ref.data();
			this.chapters=[];
		...
		chapters_refread.forEach(as=>{
			...
			setDoc(doc(firestore,"newspapers",this.newspaper_id),{popularity:this.newspaper.popularity+1},{merge:true});
		...
		methods: {
			...
			let fav= (await getDoc(doc(firestore,"newspapers",this.newspaper_id))).data().favorites;
			await updateDoc(doc(firestore,"newspapers",this.newspaper_id),{favorites:this.newspaper.favorites+1},{merge:true}); 
			}
			else 
			{
