...
export default {
  ...
  async mounted()
  {
    let newspaperref=await getDoc(doc(firestore,"posters",this.newspaper_id));
  ...
  methods:
  {
      async image_loading()
      {
    let ref_thumbnail=ref(storage,`/posters/${this.newspaper_id}/thumbnail.jpg`);
    ...
    open_newspaper()
    {
      ...
      if(this.signedin())
      {
        l=`/poster/${this.newspaper_id}/${replace_white(this.newspaper.name)}`;
      }
      else
      {
        l=`/public/poster/${this.newspaper_id}/${replace_white(this.newspaper.name)}`;
      }
      this.$router.push(l);
