...
<md-toolbar class="md-primary">
	...
	<md-button @click="mpagechooser=true" class="pages">{{gt("pages")}} {{this.page}} / {{this.note.page_number}}</md-button>
	</hide-at>
	...
	<md-button v-if="page<note.page_number" class="small" @click="next_page"><md-icon>navigate_next</md-icon></md-button>
	<hide-at breakpoint="small"> 
...
<md-toolbar class="md-primary">
	...
	<md-button v-if="page<note.page_number" @click="next_page"><md-icon>navigate_next</md-icon></md-button>
	<hide-at breakpoint="small"> 
		<md-button @click="settings"><md-icon>settings</md-icon></md-button>
	</hide-at>
	</div>
...
import logo from "@/assets/logo";
	...
	export default {
		...
		async mounted() {
			this.note_id= this.$route.params.cid;
		...
		if(!(this.member||this.admin||this.promotion)) this.back_to_home();
			 ...
			 let image_ref = ref(storage, `/notes/${this.$route.params.nid}/sheets/${this.$route.params.cid}/book.pdf`);// loading page from bucket
			 ...
			 this.title_side=title_page(`${this.note.note_name} - ${this.gt("page")} ${this.page}`);

			

			for(let i=this.page;i<(this.page+this.preloading_page_number);i++)
			{
				if(i<=Number(this.note.page_number))  await this.add_page_to_load(i);
			}
			
			/*if (this.page<=0 || this.page>Number(this.note.page_number))
			{
		...
		methods: {
			...
			this.enter_read(this.note.page_number);
			},
			back_one_page()
			{
