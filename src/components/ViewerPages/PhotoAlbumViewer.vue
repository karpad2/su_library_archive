...
<md-toolbar class="md-primary">
	...
	<md-button @click="mpagechooser=true" class="pages">{{gt("pages")}} {{this.page}} / {{this.photoalbum.page_number}}</md-button>
	</hide-at>
	...
	<md-button v-if="page<photoalbum.page_number" class="small" @click="next_page"><md-icon>navigate_next</md-icon></md-button>
	<hide-at breakpoint="small"> 
...
<md-toolbar class="md-primary">
	...
	<md-button v-if="page<photoalbum.page_number" @click="next_page"><md-icon>navigate_next</md-icon></md-button>
	<hide-at breakpoint="small"> 
		<md-button @click="settings"><md-icon>settings</md-icon></md-button>
	</hide-at>
	</div>
...
import logo from "@/assets/logo";
	...
	export default {
		...
		if(!this.member||!this.admin||!this.promotion) this.back_to_home();
			 ...
			 this.title_side=title_page(`${this.photoalbum.photoalbum_name} - ${this.gt("page")} ${this.page}`);

			

			for(let i=this.page;i<(this.page+this.preloading_page_number);i++)
			{
				if(i<=Number(this.photoalbum.page_number))  await this.add_page_to_load(i);
			}
			
			/*if (this.page<=0 || this.page>Number(this.photoalbum.page_number))
			{
		...
		methods: {
			...
			{
				let prev= ref(storage, `/photoalbums/${this.photoalbum_id}/pages/${lo}.jpg`);
				let l={
						id:lo,
			...
			this.enter_read(this.photoalbum.page_number);
			},
			back_one_page()
			{
			...
			back_to_home()
			{
				this.$router.push(`/photoalbum/${this.photoalbum_id}/${replace_white(this.$route.params.bname)}`);
