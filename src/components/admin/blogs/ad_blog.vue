...
<div >
  ...
  <md-card class="md-layout-item md-size-50 md-small-size-100" v-if="dataReady">
    <md-card-header>
        <div class="md-title">{{gt('add_new_blog')}}</div>
    </md-card-header>
     <md-card-content>
    <md-field>
      <label>{{gt('name')}}</label>
...
export default {
    ...
    async mounted()
    {
      ...
      let book_refread=await getDoc(doc(firestore,"blogs",this.book_id));
    ...
    async change()
    {
      ...
      this.book_ref=await addDoc(collection(firestore,"blogs"),this.book,{merge:true});
      this.book_id=this.book_ref.id;
      this.$router.push(`/admin/blogs/${this.book_id}`);
      }
      else
      {
        this.book_ref=updateDoc(doc(firestore,"blogs",this.book_id),this.book,{merge:true});
      }
    ...
    async upload_book_cover()
    {
      ...
      const firststorageRef = ref(storage,`blogs/${this.book_id}/thumbnail.jpg`);
