...
export default (async (req:functions.https.Request,res:functions.Response)=>  {
    ...
    var con= mysql.createConnection(mysqlConfig).connect(function (err) {
      ...
      let query=`insert into pdfs (profile,id,chapter_id,page,text) values ('${req.params.profile}','${req.params.id}','${req.params.chapter_id}','${req.params.page}',${req.params.text})`;
